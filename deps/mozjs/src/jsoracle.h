...
class Oracle {
  public:
    ...
    void init(const char* fname) {
		...
		//hotFunctionInfos = (OHotFunctionInfo**)malloc(sizeof(OHotFunctionInfo*) * noHotFunctions);

		for (uint32_t i = 0; i < noHotFunctions && fscanf(fstream, "%[^;];%d;%d\n", jsFile, &lineNo, &column) != EOF; i++)
		{
			OHotFunctionInfo* functionInfo = new OHotFunctionInfo(jsFile, lineNo, column);
			//hotFunctionInfos[i] = functionInfo;
			hotFunctionInfos.push_front(functionInfo);
		}
